...
export default async function RegressionRunDetail({ params }: { params: { id: string; runId: string } }) {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-6xl">
      <Link href={`/architectures/${params.id}/regression`} className="text-xs text-mute inline-flex items-center gap-1 hover:text-ink">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Passed" value={run.passedSteps} accent="text-accent-green" />
        <StatCard label="Failed" value={run.failedSteps} accent={run.failedSteps > 0 ? 'text-accent-red' : undefined} />
        <StatCard label="Pass rate" value={`${passRate}%`} accent="text-primary" />
      </div>

      {duration !== null && (
        <div className="text-xs text-mute">Total duration: {formatDuration(duration)}</div>
      )}
        ...
        <Card className="border-primary/30 bg-primary/5">
          ...
          <CardContent className="pt-0 space-y-2">
            ...
            {summary.recommendations.length > 0 && (
              <ul className="list-disc list-inside space-y-1 text-xs text-mute">
          ...
          <div className="divide-y divide-hairline -mx-4">
            {run.steps.map((s) => (
              <details key={s.id} className="group">
                <summary className="flex items-center gap-3 py-3 px-4 cursor-pointer hover:bg-surface-elevated/40 list-none">
                  {s.status === 'passed' ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-mute truncate">{s.service.name} · {s.type}</div>
                  </div>
                  <div className="text-xs text-mute">{formatDuration(s.duration)}</div>
                  <StatusBadge status={s.status} />
                </summary>
                <div className="bg-surface-elevated/30 px-4 py-3 space-y-2 text-xs">
                  {s.description && <div className="text-mute">{s.description}</div>}
                  {s.errorMessage && (
                    <div className="rounded-md bg-accent-red/10 border border-accent-red/30 p-2 text-accent-red">
                      {s.errorMessage}
                    </div>
                  )}
                  {s.input && (
                    <div>
                      <div className="text-mute mb-1">Input</div>
                      <pre className="bg-surface-elevated/60 p-2 rounded-md overflow-x-auto text-[11px]">{JSON.stringify(parseJson(s.input, {}), null, 2)}</pre>
                    </div>
                  )}
                  {s.actualOutput && (
                    <div>
                      <div className="text-mute mb-1">Actual output</div>
                      <pre className="bg-surface-elevated/60 p-2 rounded-md overflow-x-auto text-[11px]">{JSON.stringify(parseJson(s.actualOutput, {}), null, 2)}</pre>
                    </div>
                  )}
                </div>
              </details>
