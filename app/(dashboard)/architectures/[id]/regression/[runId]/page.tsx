  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-6xl">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Passed" value={run.passedSteps} accent="text-emerald-500" />
        <StatCard label="Failed" value={run.failedSteps} accent={run.failedSteps > 0 ? 'text-red-500' : undefined} />
          ...
          <div className="divide-y divide-border/50 -mx-4">
            {run.steps.map((s) => (
              <details key={s.id} className="group">
                <summary className="flex items-center gap-3 py-3 px-4 cursor-pointer hover:bg-muted/40 list-none">
                  {s.status === 'passed' ? (
                ...
                <div className="bg-muted/30 px-4 py-3 space-y-2 text-xs">
                  ...
                  {s.errorMessage && (
                    <div className="rounded-md bg-red-500/10 border border-red-500/30 p-2 text-red-500">
                      {s.errorMessage}
                    </div>
                  )}
                  {s.input && (
