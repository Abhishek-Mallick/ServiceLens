...
const STATE_STYLE: Record<string, string> = {
  open: 'border-emerald-500/40 text-emerald-500',
  merged: 'border-blue-500/40 text-blue-500',
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          <FileText className="h-4 w-4 text-emerald-500" /> AI fix PR
        </CardTitle>
      ...
      <CardContent>
        ...
        {opened && remediation && (
          <div className="rounded-md border border-border/50 p-3 mb-3 flex flex-wrap items-center gap-3">
          ...
          <div className="space-y-3">
            ...
            {fix.files.map((f, i) => (
              <div key={i} className="rounded-md border border-border/50 overflow-hidden">
                <div className="px-3 py-1.5 border-b border-border/50 bg-muted/30 text-xs font-mono">{f.path}</div>
                <pre className="bg-background/40 text-[12px] font-mono leading-relaxed overflow-x-auto p-3 m-0">
                  {f.patch.split('\n').map((line, j) => (
