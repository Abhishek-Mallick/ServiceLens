  ...
  return (
    <Card>
            ...
            {LEVELS.map((l) => (
              <button key={l} onClick={() => toggleLevel(l)}
                className={cn(
                  ...
                  levels.has(l) ? 'border-foreground/40 text-foreground' : 'border-border/50 text-muted-foreground line-through'
                )}>
                {l}
              </button>
            ))}
      ...
      <CardContent className="pt-0">
        <div ref={listRef} className="rounded-md border border-border/50 bg-background/40 font-mono text-[12px] leading-relaxed max-h-[70vh] overflow-y-auto">
          {visible.length === 0 && (
          ...
          {visible.map((r) => (
            <div key={r.id} className="grid grid-cols-[80px_60px_140px_1fr] gap-2 px-3 py-1 border-b border-border/50 hover:bg-border/50">
