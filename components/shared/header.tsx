...
export function Header({ user }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center gap-4 border-b border-hairline bg-canvas/70 px-6 backdrop-blur">
      <nav className="flex items-center gap-1 text-[12px] text-ash min-w-0">
        {segments.map((seg, i) => {
      ...
      <button
        onClick={openPalette}
        className="ml-auto hidden md:inline-flex max-w-sm flex-1 items-center gap-2 rounded-md border border-hairline-strong bg-surface-elevated px-3 h-9 text-[12px] text-ash hover:border-stone transition-colors"
      >
        <Search className="h-3.5 w-3.5" />
        <span>Search architectures, services, incidents…</span>
        <kbd className="ml-auto rounded border border-hairline-strong bg-hairline px-1.5 py-0.5 text-[10px] text-mute">⌘K</kbd>
      </button>
      ...
      <div className="flex items-center gap-3 border-l border-hairline pl-4">
        <div className="hidden text-right sm:block">
          ...
          <div className="text-[11px] text-ash leading-tight">{user.email}</div>
        </div>
        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-surface-elevated border border-hairline-strong text-sm font-medium text-ink">
