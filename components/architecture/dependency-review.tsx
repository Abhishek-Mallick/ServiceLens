...
const selectCls = 'h-8 rounded-md border border-input bg-background px-2 text-[12px]';
      ...
      <CardContent className="space-y-5">
        ...
        {ambiguous.length > 0 && (
          <section className="space-y-2">
            <div className="text-[11px] uppercase tracking-[0.15em] text-amber-400">Ambiguous ({ambiguous.length})</div>
            {ambiguous.map((a) => {
              const key = `amb-${a.from.id}-${a.envVar}`;
              return (
                <div key={key} className="rounded-md border border-border/60 p-3 space-y-2">
        ...
        {decisions.length > 0 && (
          <section className="space-y-1.5">
            ...
            {decisions.map((d) => (
              <div key={d.id} className="flex items-center justify-between gap-2 text-[12px] rounded-md border border-border/40 px-3 py-1.5">
                <span>
